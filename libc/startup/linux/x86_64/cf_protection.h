...
[[gnu::always_inline]] LIBC_INLINE bool enable_cf_protection_return() {
  ...
  // TODO: without [[gnu::always_inline]] syscall_impl turns into a function
  // call we cannot use it here.
  // long ret;
  // asm volatile(
  //     "syscall"
  //     : "=a" (ret)             // Output: 'ret' gets the value from the 'rax' register
  //     : "a" (SYS_arch_prctl),  // Input: 'rax' = syscall number
  //     "D" (ARCH_SHSTK_ENABLE), // Input: 'rdi' = 1st argument
  //     "S" (kernel_feature)     // Input: 'rsi' = 2nd argument
  //     : "rcx", "r11", "memory" // Clobbered: syscall clobbers rcx, r11
  //     );
